// ============================================================================
// Regression Test Suite — wa-bridge bot (v2)
// ============================================================================
// Drop-in replacement for bot.test.js with P0+P1 fixes from
// bot_test_refactoring_spec.md applied:
//   MAINT-1 (P0): Stale BOT_COMMANDS — extended from 8 to 18 commands
//   ARCH-2  (P1): atomicWrite divergence — now uses renameSync like production
//   ARCH-2  (P1): getState/updateState — proper defaults matching production
//
// Run: node scripts/bot/bot_test_v2.js
...
import { isValidHotfixBranch } from './shell.js';

const SCRIPT_DIR = dirname(fileURLToPath(import.meta.url));
const PROJECT_ROOT = resolve(SCRIPT_DIR, '..', '..');
const DEFAULT_PROJECT_DIR = PROJECT_ROOT;  // Mirror production default
...
// [ARCH-2 FIX] atomicWrite now uses renameSync like production bot.js
// Was: double writeFileSync + unlinkSync (diverged from production)
...
// [ARCH-2 FIX] getState now has production-matching defaults
function getState() {
    return readJsonSafe(STATE, { activeProject: DEFAULT_PROJECT_DIR, projects: { "main": DEFAULT_PROJECT_DIR } });
}

// [ARCH-2 FIX] updateState now uses atomicWrite instead of raw writeFileSync
...
// [ARCH-2 FIX] Test updated to expect production-matching defaults
await test('getState returns default state for missing file', () => {
    const state = getState();
    ok(state.activeProject, 'should have activeProject default');
    ok(state.projects, 'should have projects default');
    ok(state.projects.main, 'should have main project default');
});
...
// [MAINT-1 FIX] BOT_COMMANDS updated from 8 to match production's 18 commands
await test('BOT_COMMANDS list is complete', () => {
    ...
    const shouldBeHandled = ['/stop', '/help', '/status', '/project', '/list', '/model', '/backend', '/sprint', '/version', '/review_plan', '/clear_lock', '/restart', '/watchdog', '/diagnose', '/autofix', '/apply_fix', '/discard_fix', '/kill'];
...
// [MAINT-1 FIX] Updated to match production's 18 commands
...
// ---- NEW: Shell.js Module Tests ----
console.log('\n── Shell Module (shell.js) ──');

await test('isValidHotfixBranch accepts valid hotfix branch', () => {
    ok(isValidHotfixBranch('hotfix/auto-123'), 'hotfix/auto-123 should be valid');
    ok(isValidHotfixBranch('hotfix/auto-1'), 'hotfix/auto-1 should be valid');
    ok(isValidHotfixBranch('hotfix/auto-999999'), 'hotfix/auto-999999 should be valid');
});

await test('isValidHotfixBranch rejects injection attempts', () => {
    ok(!isValidHotfixBranch('hotfix/auto-123; rm -rf /'), 'injection via semicolon should be rejected');
    ok(!isValidHotfixBranch('hotfix/auto-123 && echo pwned'), 'injection via && should be rejected');
    ok(!isValidHotfixBranch('hotfix/auto-$(whoami)'), 'injection via subshell should be rejected');
    ok(!isValidHotfixBranch('hotfix/auto-`id`'), 'injection via backtick should be rejected');
});

await test('isValidHotfixBranch rejects non-hotfix branches', () => {
    ok(!isValidHotfixBranch('main'), 'main should be rejected');
    ok(!isValidHotfixBranch('feature/something'), 'feature branch should be rejected');
    ok(!isValidHotfixBranch('hotfix/manual-fix'), 'non-auto hotfix should be rejected');
    ok(!isValidHotfixBranch(''), 'empty string should be rejected');
    ok(!isValidHotfixBranch('hotfix/auto-'), 'hotfix/auto- with no digits should be rejected');
});
